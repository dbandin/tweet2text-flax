...
def parse_arguments():
    ...
    """
    # TODO: the cred file is a position required argument, it's a little
    #       bit contradictory.
    # Define arguments.
    parser = ArgumentParser(description=__doc__)
    parser.add_argument(help="Url to specific twit to retrieve text",
                        dest='tweet_url',
                        metavar='TWITTER_URL', type=str)
    parser.add_argument('-t', '--twitter-cred-file',
                        help="Path to YAML file with twitter credentials",
                        dest='tweet_creds_file',
...
def process_image(imageFile):
    # TODO: This method will required to be divided in smaller
    # and less complex methods.
...
def initialize_twitter_api(twitter_creds):
    ...
    auth = twitter_oauth(twitter_creds['consumer_key'],
...
def parse_tweet_url(tweet_url):
    ...
    # TODO: work with shorturl links (Will need to actually retrieve it to
    #       identify the full tweet url after the redirect).
    # TODO: check that is actually a valid url, right now only splits
    #       the string.
